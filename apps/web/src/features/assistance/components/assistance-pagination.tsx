"use client"

import {
  Pagination,
  PaginationContent,
  PaginationEllipsis,
  PaginationItem,
  PaginationLink,
  PaginationNext,
  PaginationPrevious,
} from "@shurokkha/ui/components/pagination"

import { useAssistanceList } from "../hooks/use-assistance-list"
import { useAssistanceQuery } from "../hooks/use-assistance-query"

/**
 * Bottom-of-list pagination strip for the assistance page.
 *
 * - Reads page + per_page from {@link useAssistanceQuery}.
 * - Reads the page data from {@link useAssistanceList} — TanStack Query
 *   shares the cache with {@link AssistanceList} so this costs nothing.
 * - Returns nothing while the list is loading or has no data.
 */
export function AssistancePagination() {
  const { search, status, type, priority, sort, dir, page, per_page, setPage } =
    useAssistanceQuery()

  const { data, isFetching } = useAssistanceList({
    search: search || undefined,
    status: status ?? undefined,
    type: type ?? undefined,
    priority: priority ?? undefined,
    sort,
    dir,
    page,
    per_page,
  })

  const meta = data?.meta
  if (!meta) return null

  const canPrev = meta.current_page > 1
  const canNext = meta.current_page < meta.last_page
  const window = buildPageWindow(meta.current_page, meta.last_page)
  const summary = buildSummary(meta)

  return (
    <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
      <span className="text-muted-foreground">{summary}</span>
      <Pagination className="mx-0 w-auto justify-end">
        <PaginationContent>
          <PaginationItem>
            <PaginationPrevious
              href="#"
              onClick={(event) => {
                event.preventDefault()
                if (!canPrev || isFetching) return
                setPage(Math.max(1, meta.current_page - 1))
              }}
              aria-disabled={!canPrev || isFetching}
              className={
                !canPrev || isFetching
                  ? "pointer-events-none opacity-50"
                  : undefined
              }
            />
          </PaginationItem>

          {window.map((entry, index) =>
            entry === "…" ? (
              <PaginationItem key={`ellipsis-${index}`}>
                <PaginationEllipsis />
              </PaginationItem>
            ) : (
              <PaginationItem key={entry}>
                <PaginationLink
                  href="#"
                  isActive={entry === meta.current_page}
                  onClick={(event) => {
                    event.preventDefault()
                    if (isFetching) return
                    setPage(entry)
                  }}
                >
                  {entry}
                </PaginationLink>
              </PaginationItem>
            )
          )}

          <PaginationItem>
            <PaginationNext
              href="#"
              onClick={(event) => {
                event.preventDefault()
                if (!canNext || isFetching) return
                setPage(meta.current_page + 1)
              }}
              aria-disabled={!canNext || isFetching}
              className={
                !canNext || isFetching
                  ? "pointer-events-none opacity-50"
                  : undefined
              }
            />
          </PaginationItem>
        </PaginationContent>
      </Pagination>
    </div>
  )
}

function buildPageWindow(current: number, last: number): Array<number | "…"> {
  if (last <= 7) return Array.from({ length: last }, (_, i) => i + 1)

  const window: Array<number | "…"> = [1]
  const left = Math.max(2, current - 1)
  const right = Math.min(last - 1, current + 1)

  if (left > 2) window.push("…")
  for (let page = left; page <= right; page++) window.push(page)
  if (right < last - 1) window.push("…")
  window.push(last)

  return window
}

function buildSummary(meta: {
  total: number
  from: number | null
  to: number | null
  per_page: number
  current_page: number
}): string {
  const total = meta.total
  const from =
    meta.from ?? (total === 0 ? 0 : (meta.current_page - 1) * meta.per_page + 1)
  const to = meta.to ?? Math.min(total, meta.current_page * meta.per_page)
  if (total === 0) return "No requests"
  return `Showing ${from.toLocaleString()}–${to.toLocaleString()} of ${total.toLocaleString()}`
}
