...
/**
 * Bottom-of-list pagination strip for the missing-persons page.
 *
 * - Reads page + per_page from {@link useMissingPersonsQuery}.
 * - Reads the page data from {@link useMissingPersonsList} — TanStack
 *   Query shares the cache with {@link MissingPersonList} so this
 *   costs nothing.
 * - Returns nothing while the list is loading or has no data.
 */
export function MissingPersonPagination() {
  const { search, status, sort, dir, page, per_page, setPage } =
    useMissingPersonsQuery()

  const { data, isFetching } = useMissingPersonsList({
