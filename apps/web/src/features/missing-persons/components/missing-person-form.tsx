...
import { missingPersonInputSchema } from "@shurokkha/contracts"
...
import { useCreateMissingPerson } from "../hooks/use-create-missing-person"
import { useUpdateMissingPerson } from "../hooks/use-update-missing-person"
...
export function MissingPersonForm({
  ...
  const createMutation = useCreateMissingPerson()
  const updateMutation = useUpdateMissingPerson()
  ...
  function handleCancel() {
    ...
    if (initial) {
      router.push(routes.account.missingPerson(initial.id))
      return
    }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const record = initial
        ...
        : await createMutation.mutateAsync({ input, photo })

      onSaved?.(record)
      if (!initial) router.push(routes.account.missingPerson(record.id))
    } catch (error) {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-6">
      <form className="space-y-6" onSubmit={handleSubmit} noValidate>
        ...
        <FormSection compact title="Person details">
          <FormGrid columns={4}>
            <FieldGroup
