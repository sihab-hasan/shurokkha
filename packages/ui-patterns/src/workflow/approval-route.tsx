...
export function ApprovalRoute({
  ...
  return (
    <Card className={cn("overflow-hidden", className)} {...props}>
      ...
      <CardContent
        className={cn("py-5", direction === "horizontal" && "overflow-x-auto")}
      >
        <ol
          className={cn(
            direction === "vertical"
              ? "space-y-0"
              : "flex min-w-max items-start gap-3"
          )}
        >
          {steps.map((step, index) => (
            ...
            >
              ...
              <Avatar className="relative z-10 size-10 shrink-0 border bg-background">
                {step.avatarUrl ? (
                  <AvatarImage src={step.avatarUrl} alt="" />
                ) : null}
                <AvatarFallback>
                  {step.initials ?? String(index + 1)}
                </AvatarFallback>
              </Avatar>
              <div
                className={cn("min-w-0", direction === "horizontal" && "mt-2")}
              >
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-semibold">{step.name}</span>
                  {step.status ? (
                    <EntityStatus tone={step.tone ?? "neutral"}>
                      {step.status}
                    </EntityStatus>
                  ) : null}
                </div>
              ...
              {direction === "horizontal" && index < steps.length - 1 ? (
                <span
                  aria-hidden="true"
                  className="absolute top-5 left-[10rem] h-px w-[4rem] bg-border"
                />
              ) : null}
            </li>
          ))}
