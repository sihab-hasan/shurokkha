...
export function AttachmentUploader({
  ...
  return (
    <div
      className={cn(
        "flex flex-col items-center justify-center rounded-xl border border-dashed bg-muted/15 text-center transition-colors",
        ...
        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
          {description ??
            [
              ...
              maxSizeBytes
                ? `Max ${(maxSizeBytes / 1024 / 1024).toFixed(0)} MB each`
                : null,
            ]
              .filter(Boolean)
              .join(" · ")}
