  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl overflow-hidden p-0">
        ...
        <div className="flex max-h-[72vh] min-h-[24rem] items-center justify-center overflow-auto bg-muted/25 p-4">
          {preview ??
            (url && isImage ? (
              <img
                src={url}
                alt={name}
                className="max-h-[66vh] max-w-full rounded-lg object-contain shadow-xs"
              />
            ) : url && isPdf ? (
              <iframe
                src={url}
