...
import {
  ...
  CollapsibleContent,
  CollapsibleTrigger,
} from "@shurokkha/ui/components/collapsible"
...
function TreeNode({
  ...
  const row = (
    <button
      ...
      className={cn(
        "flex min-h-9 min-w-0 flex-1 items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm transition-colors outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring/35 disabled:pointer-events-none disabled:opacity-50",
...
export function HierarchicalPicker({
  ...
  return (
    <div
      ...
      className={cn(
        "max-h-80 space-y-1 overflow-y-auto rounded-xl border bg-card p-2",
        className
      )}
    >
      {nodes.map((node) => (
